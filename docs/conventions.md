...
- 한 파일에 한 컴포넌트.
- 한 컴포넌트에 되도록 하나의 역할.
- 하향식("Top-Down") 컴포넌트 배치: 파일의 메인이 되는 대표 컴포넌트를 최상단에 노출시키고, 이를 지원하는 'defaultMarketProfile' 등과 같은 정적 보조 상수, 설정 변수, static 헬퍼 함수 등은 파일의 최하단에 모아서 선언하도록 유도합니다. (신문 기사 스타일 구조 적용)
